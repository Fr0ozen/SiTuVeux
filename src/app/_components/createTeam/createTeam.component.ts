    ...
    onSubmit() {
        ...
        if (!this.createTeamForm.invalid) {
            ...
            if (new Set(playerPseudoList).size === 5) {
                this.teamService.createTeam(this.user, new Team(this.f.name.value, this.f.origin.value, Array.from(playerList.values()))).pipe(first()).subscribe(data => {
                ...
                this.error = 'Les joueurs ne peuvent pas être sélectionné plus d\'une fois';
            }
        }
    }
